// 241128_161990.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
예를 들어 wallpaper = [".#...", "..#..", "...#."]인 바탕화면을 정리해보겠습니다.
S(0, 1)에서 E(3, 4)로 드래그하면 세 개의 파일이 모두 선택되므로 드래그 한 거리 (3 - 0) + (4 - 1) = 6을 최솟값으로 모든 파일을 선택 가능합니다.
...
std::vector<int> solution(std::vector<std::string> wallpaper)
{
    ...
    const int Row = static_cast<int>(wallpaper.size());
    const int Column = static_cast<int>(wallpaper[0].size());
